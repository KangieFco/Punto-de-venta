...
public class TicketService : ITicketService
{
    ...
    public async Task<TicketDto> GetBySaleIdAsync(int saleId)
    {
        var ticket = await _db.Tickets
            ...
            ?? throw new NotFoundException("Ticket para la venta", saleId);
    ...
    public async Task<TicketDto> PrintAsync(int id, int userId)
    {
        ...
        ticket.PrintedCount++;
        ticket.LastPrintedAt = DateTime.UtcNow;
        await _db.SaveChangesAsync();

        var dto          = ToDto(ticket);
        dto.TicketText   = await GenerateTextAsync(ticket.SaleId);
        return dto;
    }
    ...
    public async Task<string> GenerateTextAsync(int saleId)
    {
        ...
        var storeName = _config["Store:Name"]    ?? "MI TIENDA";
        ...
        var width     = 32; // caracteres para 58mm; usa 48 para 80mm
        ...
        sb.AppendLine(ColLine($"Pagó ({sale.PaymentMethod}):",
